'''
Sample Usage:-
python calibration.py --dir calibration_checkerboard/ --square_size 0.009
'''
...
def calibrate(dirpath, square_size, width, height, visualize=False):
    ...
    for fname in images:
        ...
        ret, corners = cv2.findChessboardCorners(gray, (width, height), None) # 棋盘格每行每列角点个数
        print(ret)
...
if __name__ == '__main__':
    ...
    ap.add_argument("-w", "--width", type=int, help="Width of checkerboard (default=9)",  default=8) # 棋盘格每行每列角点个数
    ap.add_argument("-t", "--height", type=int, help="Height of checkerboard (default=6)", default=5) # 棋盘格每行每列角点个数
